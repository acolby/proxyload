...
/*
goes throught the target directory fins all interface and type files of items and merges them into an object that can easily be consumed by target applications
*/
  ...
  for (const typeName of typeNames) {
    ...
    for (const itemName of itemNames) {
      ...
      const types = fs.readFileSync(
        path.resolve(params.dir, typeName, itemName, "types.ts"),
        "utf-8"
      );
...
async function _createRolledUpIndexFile(params: { dir: string }) {
  ...
  for (const typeName of typeNames) {
    ...
    for (const itemName of itemNames) {
      ...
      importLines.push(
        `import type { ${fullname} } from "./${typeName}/${itemName}/types.ts";`
      );
